...
impl HarperEngine {
    ...
    pub fn lint(&self, text: String, _language: String) -> Vec<LintResult> {
        ...
        let mut linter = harper_core::linting::LintGroup::new_curated(dict, harper_core::Dialect::American);
        let lints = linter.lint(&doc);
        
        lints.into_iter().map(|lint| {
            // Convert harper-core char indices to UTF-16 code unit indices for Android
            let start_utf16: usize = text.chars().take(lint.span.start).map(|c| c.len_utf16()).sum();
            let end_utf16: usize = text.chars().take(lint.span.end).map(|c| c.len_utf16()).sum();
            
            LintResult {
                start_utf16: start_utf16 as u32,
                end_utf16: end_utf16 as u32,
                message: lint.message,
                suggestions: lint.suggestions.into_iter().map(|s| {
                    match s {
                        harper_core::linting::Suggestion::ReplaceWith(r) => r.iter().collect::<String>(),
                        _ => String::new()
                    }
