...
class InputBoundsTest {
    ...
    private fun makeNode(text: String): AccessibilityNodeInfo = mock {
    ...
    private fun makeEvent(node: AccessibilityNodeInfo): AccessibilityEvent = mock {
        on { eventType } doReturn AccessibilityEvent.TYPE_VIEW_TEXT_CHANGED
        on { source } doReturn node
    }

    @Test
    fun testTextExactly50kCharsIsAnalyzed() = runTest {
    ...
    @Test
    fun testTextOver50kCharsIsDropped() = runTest {
        val grammarRepository = mock<GrammarRepository>()
        val tracker = EditableNodeTracker(this, grammarRepository)
